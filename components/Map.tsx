      ...
      <MapView
        ...
        onRegionChangeComplete={(newRegion) => {
          ...
          const constrainedRegion = {
            ...newRegion,
            latitude: Math.min(
              Math.max(newRegion.latitude, bangladeshBounds.southWest.latitude),
              bangladeshBounds.northEast.latitude
            ),
            longitude: Math.min(
              Math.max(
                newRegion.longitude,
                bangladeshBounds.southWest.longitude
              ),
              bangladeshBounds.northEast.longitude
            ),
          };
      ...
      >
        {posts.map((post) => (
          <Marker
            ...
            image={
              categories.find((category) => category.title === post.category)
                ?.image
            }
