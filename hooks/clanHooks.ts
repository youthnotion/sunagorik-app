...
export function useMyClans() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const clans = (data ?? [])
        .map((r: any) => (Array.isArray(r.clan) ? r.clan[0] : r.clan))
        .filter(Boolean) as Clan[];
...
export function useCreateClan() {
  ...
  return useMutation({
    mutationFn: async (payload: { name: string; description?: string; logo_url?: string }) => {
      ...
      // max clans membership (created + joined)
  ...
  return useMutation({
    mutationFn: async (clanId: string) => {
      ...
      const { error } = await supabase
        .from("clan_join_requests")
        .insert({ clan_id: clanId, user_id, status: "pending" });
      if (error) throw error;
    },
    onSuccess: (_, clanId) => {
...
export function useManageJoinRequest() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      console.log(fetchErr, "Request error:");
